...
public class SfxManager : Singleton<SfxManager>
{
	...
	public void Play (string name, float pitchVariance = 0, bool loop = false)
	{
		if (sfx_list.ContainsKey (name)) {
			if (pitchVariance != 0)
				m_audio_source.pitch = 1 + Random.Range (-pitchVariance, pitchVariance);

			m_audio_source.clip = sfx_list [name];
			m_audio_source.Play ();
			m_audio_source.loop = loop;
